   ...
   private static float getQWithParamInfo(WeightedLanguage lang, String val)
   {
      try
      {
         if (val != null)
         {
            ...
            if (rtn > 1.0F)
               throw new RuntimeException("Accept-Language q value cannot be greater than 1.0 " + lang.toString());
            return rtn;
         }
      }
      catch (NumberFormatException e)
      {
         throw new RuntimeException("MediaType q parameter must be a float: " + lang);
      }
      return 1.0f;
   }
